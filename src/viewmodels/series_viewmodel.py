...
class SeriesViewModel:
    ...
    def get_sorted_books(self):
        ...
        if self._sorted_books is None:
            ...
            if not books:
                ...
                self._sorted_books = sorted(books, key=natural_sort_key)

        return self._sorted_books
    ...
    def find_book_by_order(self, order):
        """
        指定した順番の書籍を取得する。
    ...
    def get_next_available_order(self):
        ...
        max_order = 0
        for book in books:
            if book.series_order is not None and book.series_order > max_order:
                max_order = book.series_order
